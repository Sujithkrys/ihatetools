...
export function TransformChip({ beforeText, afterText, arrowText, isStacked, category }: TransformChipProps) {
  ...
  if (!afterText) {
    return (
      <div className="flex items-center gap-[11px] mb-[18px]">
        <div className={`w-[33px] h-[40px] border-[1.5px] border-ink rounded-[5px] flex items-center justify-center font-sans font-semibold text-[9px] shrink-0 ${bgClass}`}>
          {beforeText}
        </div>
      </div>
    );
  ...
  return (
    <div className="flex items-center gap-[11px] mb-[18px]">
      {/* Before */}
      <div className={`relative w-[33px] h-[40px] border-[1.5px] border-ink rounded-[5px] bg-bg flex items-center justify-center font-sans font-semibold text-[9px] text-grey shrink-0 z-10`}>
        {beforeText}
        {isStacked && (
      ...
      {/* Arrow */}
      {arrowText && (
        <div className="font-sans font-semibold text-[14px] text-pink">
          {arrowText}
        </div>
      )}
      ...
      {Array.isArray(afterText) ? (
        <div className="flex items-center gap-1">
          {afterText.map((text, i) => (
             <div key={i} className={`w-[25px] h-[31px] border-[1.5px] border-ink rounded-[5px] flex items-center justify-center font-sans font-semibold text-[8px] shrink-0 ${i === 0 ? bgClass : 'bg-bg text-grey'}`}>
               {text}
             </div>
          ))}
        </div>
